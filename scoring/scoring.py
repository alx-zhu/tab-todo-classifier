...
def generic_average_relevance_score(relevance_fn, args: list, n=4, threads=4, **kwargs):
    scores = []
    
    # Use ThreadPoolExecutor to make repeated parallel calls for the same pair
    with ThreadPoolExecutor(max_workers=threads) as executor:
        futures = [executor.submit(relevance_fn, *args, **kwargs) for _ in range(n)]
...
def generic_feedback_relevance_score(relevance_fn, args: list, kwargs: dict = {}, repeat=3):
    prev_score = None
    prev_explanation = None
    
    for _ in range(repeat):
        score, explanation = relevance_fn(*args, prev_score=prev_score, prev_explanation=prev_explanation, **kwargs)
        if score is not None:
            prev_score = score
...
def get_score_tab_to_task(tab_name, task_name, prev_score=None, prev_explanation=None, temperature=0.3):
...
def get_score_tab_to_task_with_content(tab_url, tab_content, task_name, prev_score=None, prev_explanation=None, temperature=0.3):
    feedback_injection = (f"""
    Previously another assistant gave a score of {prev_score} with the following explanation: {prev_explanation}.
    Use this as context for your evaluation, but adjust as needed based on your own judgment.
    If your score differs from the previous one, briefly justify your score and explain your reasoning.
    ...
    prompt = f"""
    Score how relevant the browser tab at the URL "{tab_url}" is to the task "{task_name}". 
    {feedback_injection}
...
def get_score_tab_to_history(tab_name, history, prev_score=None, prev_explanation=None, temperature=0.3):
    feedback_injection = (f"""
    Previously another assistant gave a score of {prev_score} with the following explanation: {prev_explanation}.
    Use this as context for your evaluation, but adjust as needed based on your own judgment.
    If your score differs from the previous one, briefly justify your score and explain your reasoning.
...
def average_tab_to_task_relevance_with_content(tab_url, tab_content, task_name, temperature=0.3):
    return generic_average_relevance_score(get_score_tab_to_task_with_content, [tab_url, tab_content, task_name], temperature=temperature)
...
def tab_relevance_page(tab_to_task_fn, tab_to_history_fn, use_tab_content=False):
    st.title("Tab-Task Relevance Scorer")

    st.subheader("Tab to Task Relevance")

    # Input fields for lists of tabs and tasks
    tabs = st.text_area("Enter tab URLs (one per line):", value="https://www.youtube.com/watch?v=aircAruvnKk&list=PLZHQObOWTQDNU6R1_67000Dx_ZCJB-3pi\nhttps://jobs.ashbyhq.com/kleinerperkinsfellows/97a02c44-099d-490d-9353-136e7a25fb4c\n")
        ...
        if tab_list and task_list:
            ...
            for task_name in task_list:
                for tab_name in tab_list:
                    if use_tab_content:
                        score = tab_to_task_fn(tab_name, fetch_tab_content(tab_name), task_name)
                    else:
...
if __name__ == "__main__":
    tab_relevance_page(average_tab_to_task_relevance_with_content, average_tab_to_history_relevance, use_tab_content=True)
